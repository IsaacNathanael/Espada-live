...
def test_operations_dashboard_is_self_contained_and_interactive(tmp_path: Path) -> None:
    ...
    candidate = {
        "mmsi": "999000001", "vessel_name": "Blinded synthetic 999000001", "rank": 1,
    ...
    _write(case / "run/sar/slick_observation.geojson", {"type": "FeatureCollection", "features": [{"type": "Feature", "properties": {}, "geometry": {"type": "Polygon", "coordinates": [[[9.2,43.2],[9.4,43.2],[9.4,43.4],[9.2,43.2]]]}}]})
    ...
    pd.DataFrame({"timestamp_utc": ["2018-10-07T19:00:00Z","2018-10-08T05:00:00Z"], "mmsi": ["999000001"]*2, "vessel_name": ["Blinded synthetic 999000001"]*2, "longitude": [9.3,9.35], "latitude": [43.3,43.31]}).to_csv(run / "ais/ais_normalized.csv", index=False)
    ...
    assert "HYBRID SIMULATION" in page
