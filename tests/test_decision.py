...
def _write_case(root: Path, *, aligned: bool = True, separated: bool = True, sensitivity: bool = True) -> None:
    ...
    candidates = [
        {"rank": 1, "mmsi": "CAND-1", "total_score": 0.78, "forward_error_km": 0.74, "data_quality": 0.49},
        {"rank": 2, "mmsi": "CAND-2", "total_score": second, "forward_error_km": 2.0, "data_quality": 0.8},
    ]
...
def test_decision_limits_case_without_sensitivity_run(tmp_path: Path) -> None:
    _write_case(tmp_path, sensitivity=False)
    result = evaluate_case_decision(tmp_path, tmp_path / "decision")
