...
def generate_evidence_dossier(case_root: Path, output_dir: Path) -> dict[str, object]:
    ...
    if historical:
        ...
        if sensitivity:
            historical_section += f"""<article class='panel proof-card'><div class='eyebrow'>Physics sensitivity audit</div><div class='proof-answer'>{html.escape(str(sensitivity.get('verdict', 'NOT RUN')))}</div><p>{html.escape(str(sensitivity.get('interpretation', '')))}</p><div class='proof-kpis'><span><small>Top-1 stability</small><b>{_percent(sensitivity.get('top_1_rate'))}</b></span><span><small>Top-3 retention</small><b>{_percent(sensitivity.get('top_3_rate'))}</b></span><span><small>Worst rank</small><b>#{html.escape(str(sensitivity.get('worst_rank', '—')))}</b></span></div>{f"<img src='{sensitivity_uri}' alt='Physics sensitivity rank matrix'>" if sensitivity_uri else ''}<p class='disclosure'>One-case robustness, not population accuracy. Assumptions varied: spill age, current magnitude and windage.</p></article>"""
