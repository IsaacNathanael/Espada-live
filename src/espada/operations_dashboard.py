...
def build_operations_dashboard(project_root: Path, output_path: Path) -> dict[str, object]:
    ...
    run = case / "counterfactual_ais/run"
    ...
    slick_path = run / "sar/slick_observation.geojson"
    ...
    payload = {
        "case": {
            "id": "corsica-2018-hybrid",
            "name": "Corsica · 2018",
            "mode": "HYBRID SIMULATION",
        ...
        "decision": decision,
        "digitalTwin": twin,
        "dossier": "../external_validation/corsica_2018/counterfactual_ais/run/dossier/evidence_dossier.html",
        "disclosure": (
            "Real Sentinel-1 image, real Copernicus/Open-Meteo forcing and 76 real GFW background tracks; "
...
</style></head><body><div class="app"><header><div class="brand"><div class="mark">E</div>ESPADA</div><div class="case"><b id="caseName"></b><span>Historical evidence replay</span></div><div class="badge" id="modeBadge"></div><div class="spacer"></div><span class="server" id="server">SAVED EVIDENCE MODE</span><button class="soft" id="details">Case details</button><button class="primary" id="run">▶ Run analysis</button></header>
<main class="workspace"><section class="map-card" id="mapCard"><img class="sar" id="sar" alt="Real Sentinel-1 input"><div class="map-shade"></div><svg class="overlay" id="overlay" viewBox="0 0 1000 650" preserveAspectRatio="none"><g id="grid"></g><g id="slickLayer"></g><g id="particleLayer"></g><g id="releaseLayer"></g><g id="trackLayer"></g><g id="shipLayer"></g></svg><div class="map-tools"><button class="tool" id="layers">☷ Layers</button><button class="tool" id="satellite" aria-pressed="true">◐ Satellite</button><button class="tool" id="reset">↺ Reset</button></div><div class="popover" id="layerMenu"><label><input type="checkbox" data-layer="slick" checked> Oil slick</label><label><input type="checkbox" data-layer="reverse" checked> Reverse drift</label><label><input type="checkbox" data-layer="ships" checked> Ships</label><label><input type="checkbox" data-layer="tracks"> Selected track</label></div><div class="map-status"><b id="mapStatus">REAL SENTINEL-1 INPUT</b><span id="mapSub">Select Run analysis to trace the slick</span></div><div class="legend"><span><i class="dot" style="background:#52e2dc"></i>Ships</span><span><i class="dot" style="background:#ffbd59"></i>Oil</span><span><i class="dot" style="background:#72f1e6"></i>Probable origin</span></div><div class="tooltip" id="tooltip"></div><div class="phase" id="phase"><div>01 · Detect</div><div>02 · Reverse</div><div>03 · Match</div><div>04 · Verify</div></div></section>
<aside class="side"><div class="side-head"><div><div class="eyebrow">INVESTIGATION SHORTLIST</div><h2>Candidate vessels</h2></div><div class="count"><span id="candidateCount"></span><br>Top 3 shown</div></div><div class="side-scroll"><div class="candidates" id="candidateList"></div><button class="more" id="more">Show more candidates</button><div class="detail" id="detail"><div class="detail-placeholder">Run the analysis or select a ship to inspect its evidence.</div></div></div></aside></main>
...
function init(){ $('caseName').textContent=DATA.case.name;$('modeBadge').textContent=DATA.case.mode;$('sar').src=DATA.satelliteImage;$('candidateCount').textContent=DATA.case.candidateCount+' compared';$('coverage').textContent=(DATA.case.candidateCount-1)+' real + 1 synthetic';$('startTime').textContent=new Date(minT).toLocaleString([],{month:'short',day:'numeric',hour:'2-digit'});$('endTime').textContent=new Date(maxT).toLocaleString([],{month:'short',day:'numeric',hour:'2-digit'});grid();slick();particles();ships();renderCandidates();setTime(releaseT);health()}
...
function tip(e,id){const t=DATA.tracks.find(x=>x.mmsi===id),box=$('tooltip');box.innerHTML=`<b>${esc(t.name)}</b><span>${t.rank?'Rank #'+t.rank+' · <em>'+pct(t.score)+'</em> evidence':'Background traffic'}</span><span>${t.synthetic?'Synthetic observability track':'Pseudonymized historical AIS'}</span><span>Click for evidence</span>`;box.classList.add('show');moveTip(e)}function moveTip(e){const card=$('mapCard').getBoundingClientRect(),box=$('tooltip');box.style.left=Math.min(card.width-205,e.clientX-card.left+12)+'px';box.style.top=Math.min(card.height-95,e.clientY-card.top+12)+'px'}
function renderCandidates(){const list=DATA.candidates.slice(0,showCount);$('candidateList').innerHTML=list.map(t=>`<button class="candidate ${selected===t.mmsi?'active':''}" data-id="${t.mmsi}"><span class="rank">${t.rank}</span><span><b>${esc(t.name)}</b><small>${t.synthetic?'Controlled synthetic source':'Pseudonymized AIS vessel'} · ${num(t.forwardError,2)} km replay</small></span><span class="score">${pct(t.score)}</span></button>`).join('');document.querySelectorAll('.candidate').forEach(b=>b.onclick=()=>select(b.dataset.id));$('more').textContent=showCount===3?'Show more candidates':'Show Top 3 only'}
function select(id){selected=id;const t=DATA.tracks.find(x=>x.mmsi===id);document.querySelectorAll('.ship').forEach(s=>{s.classList.toggle('selected',s.dataset.id===id);s.classList.toggle('dim',s.dataset.id!==id)});renderCandidates();drawTrack(t);$('detail').innerHTML=`<div class="detail-top"><div><div class="eyebrow">${t.rank?'RANK #'+t.rank:'TRAFFIC TRACK'}</div><h3>${esc(t.name)}</h3><div class="detail-id">ID ${esc(t.mmsi)}</div></div><div class="score">${pct(t.score)}</div></div><div class="metrics"><div class="metric"><span>Origin presence</span><b>${pct(t.presence)}</b><div class="bar"><i style="width:${(t.presence||0)*100}%"></i></div></div><div class="metric"><span>Forward agreement</span><b>${pct(t.forward)}</b><div class="bar"><i style="width:${(t.forward||0)*100}%"></i></div></div><div class="metric"><span>Track quality</span><b>${pct(t.quality)}</b></div><div class="metric"><span>AIS gap</span><b>${esc((t.silence||'not evaluated').replaceAll('_',' '))}</b></div></div><div class="detail-actions"><button id="replayVessel">Replay track</button><button id="why">Why ranked?</button><a href="${DATA.dossier}" target="_blank">Dossier ↗</a></div><div class="notice">Candidate evidence supports analyst review only. It is not a guilt probability.</div>`;$('replayVessel').onclick=()=>{setTime(Math.max(minT,Date.parse(t.bestTime||DATA.case.releaseTime)-7200000));if(!playing)togglePlay()};$('why').onclick=()=>why(t)}
...
async function run(){if(ran)resetRun();ran=true;$('run').disabled=true;$('phase').classList.add('show');$('mapStatus').textContent='ANALYSIS RUNNING';phase(0);document.querySelectorAll('.slick').forEach(x=>x.classList.add('show'));runTimer.push(setTimeout(()=>{phase(1);$('mapStatus').textContent='REVERSING DRIFT';animateParticles();setTime(releaseT)},650));let result=null;try{if(engine){const r=await fetch('/api/run-operations',{method:'POST'});if(!r.ok)throw new Error('engine response');result=await r.json()}}catch(e){result=null}runTimer.push(setTimeout(()=>{phase(2);document.querySelectorAll('.release').forEach(x=>x.classList.add('show'));$('mapStatus').textContent='MATCHING VESSELS'},3200));runTimer.push(setTimeout(()=>{phase(3);$('mapStatus').textContent='FORWARD VERIFYING'},4100));runTimer.push(setTimeout(()=>{phase(4,'done');$('mapStatus').innerHTML='PRIORITY ANALYST REVIEW';$('mapSub').textContent=(result?'Fresh local ranking':'Saved evidence replay')+' · '+DATA.case.candidateCount+' candidates';select(DATA.candidates[0].mmsi);$('run').disabled=false;$('run').textContent='Run again'},4900))}
function resetRun(){runTimer.forEach(clearTimeout);runTimer=[];playing=false;showCount=3;$('play').textContent='▶';$('run').disabled=false;$('run').textContent='▶ Run analysis';document.querySelectorAll('.slick,.particle,.release').forEach(x=>{x.classList.remove('show');x.style.display='';if(x.classList.contains('particle'))x.removeAttribute('transform')});$('shipLayer').style.display='';$('trackLayer').style.display='';$('phase').classList.remove('show');$('layerMenu').classList.remove('show');document.querySelectorAll('[data-layer]').forEach(c=>c.checked=c.dataset.layer!=='tracks');$('satellite').setAttribute('aria-pressed','true');$('sar').style.opacity='.68';$('mapStatus').textContent='REAL SENTINEL-1 INPUT';$('mapSub').textContent='Select Run analysis to trace the slick';selected=null;document.querySelectorAll('.ship').forEach(s=>s.classList.remove('selected','dim'));$('trackLayer').innerHTML='';$('detail').innerHTML='<div class="detail-placeholder">Run the analysis or select a ship to inspect its evidence.</div>';renderCandidates();setTime(releaseT)}
...
function details(){modal(DATA.case.name,`<div class="fact"><span>Input image</span><b>Sentinel-1 VV · ${DATA.satellite.acquisition_time_utc}</b></div><div class="fact"><span>Oil detection</span><b>V6 attention U-Net · human reviewed</b></div><div class="fact"><span>Environment</span><b>Copernicus currents + Open-Meteo wind</b></div><div class="fact"><span>Traffic</span><b>76 real historical tracks + 1 synthetic source</b></div><div class="fact"><span>Decision</span><b>${esc(DATA.case.decision.replaceAll('_',' '))}</b></div><div class="fact"><span>Digital-twin Top-3</span><b>${pct(DATA.digitalTwin.top_3_rate)}</b></div><div class="disclosure">${esc(DATA.disclosure)} This hybrid case is a source-observability experiment, not a real accusation.</div>`,'PROVENANCE & CLAIM BOUNDARY')}
...
$('run').onclick=run;$('reset').onclick=resetRun;$('play').onclick=togglePlay;$('time').oninput=e=>setTime(minT+Number(e.target.value)/1000*(maxT-minT));$('layers').onclick=()=> $('layerMenu').classList.toggle('show');$('satellite').onclick=e=>{const on=e.currentTarget.getAttribute('aria-pressed')==='true';e.currentTarget.setAttribute('aria-pressed',String(!on));$('sar').style.opacity=on?'.18':'.68'};$('more').onclick=()=>{showCount=showCount===3?10:3;renderCandidates()};$('details').onclick=details;$('close').onclick=()=> $('modal').close();document.querySelectorAll('[data-env]').forEach(b=>b.onclick=()=>b.dataset.env==='coverage'?modal('AIS coverage',`<div class="fact"><span>Compared</span><b>${DATA.case.candidateCount} candidate tracks</b></div><div class="fact"><span>Real background</span><b>76 GFW vessel-presence tracks</b></div><div class="fact"><span>Synthetic</span><b>1 disclosed source-consistent track</b></div><div class="disclosure">Missing AIS remains missing evidence. Silence never increases attribution score.</div>`,'DATA QUALITY'):envDetails(b.dataset.env));document.querySelectorAll('[data-layer]').forEach(c=>c.onchange=()=>{const name=c.dataset.layer;if(name==='slick')document.querySelectorAll('.slick').forEach(x=>x.style.display=c.checked?'':'none');if(name==='reverse'){document.querySelectorAll('.particle,.release').forEach(x=>x.style.display=c.checked?'':'none')}if(name==='ships')$('shipLayer').style.display=c.checked?'':'none';if(name==='tracks')$('trackLayer').style.display=c.checked?'':'none'});init();
