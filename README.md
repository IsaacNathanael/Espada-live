...
It reuses the validated artifacts, starts the local evidence server, and opens the command center at `http://127.0.0.1:4173/out/judge_demo/index.html`. **Start Presenter Mode** provides a timed five-step explanation with direct evidence links. Use `-RecomputeValidation` only when you intentionally want to rerun all three validation layers. Comparative candidate scores shown in the reports are evidence-fit scores, not guilt probabilities.
...
For a stronger simulation, `scripts\run_digital_twin.ps1` selects pseudonymized real vessel tracks, creates hidden synthetic releases with particle-local Copernicus currents and time-varying wind, models a continuous release plus diffusion, applies AIS dropout/position-noise and physics-mismatch stresses, and scores recovery only after inference. The same spatial-current engine is used for generation and reconstruction. It is a controlled digital twin—not real-spill attribution accuracy.
...
Remove `-PlanOnly` to discover and crop Sentinel-1, collect delayed historical vessel presence, and download aligned wind and surface currents. The assembler creates `out\incidents\incident_001\case.json`; run that file through `scripts\run_case.ps1`. The generated case retains the full Copernicus NetCDF subset: every reverse particle samples a current at its own position using bilinear spatial and linear temporal interpolation. Wind remains a time-varying point estimate. For incidents newer than roughly 96 hours, supply a licensed/current AIS CSV with `-ExistingAisCsv` because Global Fishing Watch is delayed.
...
The case pipeline now integrates time-varying wind and particle-local, spatially varying Copernicus currents. The incident assembler automatically downloads the public-domain [Natural Earth 1:10m land polygons](https://www.naturalearthdata.com/downloads/10m-physical-vectors/10m-land/), clips them around the case and records the archive hash. A custom polygonal land GeoJSON can also be supplied as `inputs.land_mask` in a case file (or `-LandMask` to the runner); particle steps ending on land are then rejected as a lightweight zero-flux boundary. It is still an investigative approximation rather than an operational ocean forecast: wind is point-sampled, narrow land crossings, beaching/resuspension and oil weathering are not yet modelled, and particles leaving the downloaded current subset use its nearest boundary cell.

Run `scripts\run_physics_benchmark.ps1` to compare ESPADA and OpenDrift 1.14.11 on the same real Corsica Copernicus Marine current subset and time-varying Open-Meteo wind. The benchmark evaluates five forward and five reverse deterministic trajectories and writes `out\physics_benchmark\physics_benchmark.html`. Diffusion and land interaction are disabled in both engines so it verifies deterministic current-and-wind numerical parity, not forecast skill or real-spill attribution accuracy. Regional observations and richer oil-weathering physics remain production work.

The current prototype uses the lightweight ESPADA solver in the incident pipeline and treats OpenDrift as the independent numerical reference. Moving the complete case runner onto OpenDrift readers remains the production architecture goal recorded in `docs\DECISIONS.md`.
