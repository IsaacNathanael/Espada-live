...
Run `scripts\run_physics_benchmark.ps1` to compare ESPADA and OpenDrift 1.14.11 on the same real Corsica Copernicus Marine current subset. The benchmark evaluates five forward and five reverse deterministic trajectories and writes `out\physics_benchmark\physics_benchmark.html`. It isolates particle-local current interpolation by disabling wind, diffusion and land interaction in both engines; passing it verifies numerical parity, not forecast skill or real-spill attribution accuracy. Regional observations and richer oil-weathering physics remain production work.
